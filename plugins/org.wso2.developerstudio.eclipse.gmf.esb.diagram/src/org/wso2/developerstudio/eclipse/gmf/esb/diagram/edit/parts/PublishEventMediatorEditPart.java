    ...
    public void notifyChanged(Notification notification) {
        if (this.getModel() instanceof CSSNodeImpl) {
            ...
            if (model.getElement() instanceof PublishEventMediatorImpl) {
                PublishEventMediatorImpl publishMediatorDataModel = (PublishEventMediatorImpl) model.getElement();
                org.wso2.carbon.mediator.publishevent.PublishEventMediator publishMediator = null;
                try {
                    publishMediator = PublishEventMediatorTransformer
                            .createPublishEventMediator((PublishEventMediator) publishMediatorDataModel);

                    PublishEventMediatorSerializer publishEventMediatorSerializer = new PublishEventMediatorSerializer();
                    OMElement omElement = publishEventMediatorSerializer.serializeSpecificMediator(publishMediator);

                    if (StringUtils
                            .isEmpty(MediatorValidationUtil.validateMediatorsFromOEMElement(omElement, "publishEvent"))) {
